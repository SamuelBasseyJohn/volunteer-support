"use client";
/**
 * VIEW LAYER — ManageEvent Component
 *
 * Admin / org view for a single event. Shows all applicants and
 * lets admins/orgs accept, reject, and confirm volunteer attendance.
 */
...
const EventManagement = ({ id }: { id: string }) => {
  ...
  const { data: event, isLoading, isFetched, refetch } =
    api.event.getEvent.useQuery({ id });
      ...
      {isFetched && (
        <div>
          ...
          <div className="max-w-4xl mx-auto mt-4">
            <h2 className="my-3 text-center text-3xl font-extrabold text-gray-800">
              {event?.title}
            </h2>
            <div className="flex flex-wrap gap-4 justify-center my-2 text-gray-700 text-lg">
              <span>
                <strong>Accepted:</strong> {confirmed}/{event?.max_participants}
              </span>
          ...
          <div className="max-w-4xl mx-auto mt-6">
            <h3 className="my-3 text-xl text-center font-bold text-gray-900">
            ...
            {event?.signups.length === 0 ? (
              ...
              <ul className="space-y-3">
                {event?.signups.map((signup) => (
                  <li
                    key={signup.id}
                    className="flex flex-col gap-3 rounded-xl bg-white/20 p-4 backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div>
                      ...
                      {signup.user.skills && signup.user.skills.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-1">
                          {signup.user.skills.map((skill) => (
                            <span
                              key={skill}
                              className="rounded-full bg-white/30 px-2 py-0.5 text-xs text-white"
                            >
                              {skill}
                            </span>
                          ))}
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      {signup.status === Status.PENDING && (
                        <>
                          <button
                            onClick={() =>
                              accept.mutate({ applicationId: signup.id })
                            }
                          ...
                          <button
                            onClick={() =>
                              reject.mutate({ applicationId: signup.id })
                            }
                      ...
                      {signup.status === Status.CONFIRMED &&
                        !signup.attendanceConfirmed && (
                          <button
                            onClick={() =>
                              confirmAttendance.mutate({ signupId: signup.id })
                            }
          ...
          <div className="max-w-4xl mx-auto mt-10">
            ...
            {event && <EventChat eventId={event.id} />}
          </div>
        </div>
      )}
      {isLoading && (
        <p className="text-center my-5 text-white text-xl font-semibold">
          Loading Event...
        </p>
      )}
    </div>
